...
class Node():
  ...
  def __init__(self, host, port, max_listen, role, withrandom=False):
    ...
    # Hashes of the broadcasted messages and blocks
    self.hashes_message = set()
    self.hashes_block = set()
  ...
  def processBlock(self, request):
    timestamp_sent, host, port, hash_prev, timestamp_block, merkle_root = request.split(':')
    request_hash = helper.getHash(request)
    if request_hash not in self.hashes_block:
      # Create a block
      block = Block(hash_prev, timestamp_block, merkle_root)
      block_dict = {
        'block': block,
        'hash': block.getBlockHash()
      }
      # Add the hash of the request in the set of hashes 
      self.hashes_block.add(request_hash) 
